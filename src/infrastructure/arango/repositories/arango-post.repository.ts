...
export class ArangoPostRepository implements IPostRepository {
  ...
  async findById(id: string): Promise<VkPost | null> {
    ...
    return new VkPost(
      doc.id,
      doc.ownerId ?? doc.owner_id,
      doc.fromId ?? doc.from_id,
      doc.text ?? "",
      doc.date,
    );
  }
  ...
  async save(post: VkPost): Promise<void> {
    await this.db.query(aql`
      UPSERT { id: ${post.id} }
      INSERT { id: ${post.id}, ownerId: ${post.ownerId}, fromId: ${
        post.fromId
      }, text: ${post.text}, date: ${post.date} }
      UPDATE { ownerId: ${post.ownerId}, fromId: ${post.fromId}, text: ${
        post.text
      }, date: ${post.date} }
