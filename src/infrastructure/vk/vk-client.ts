...
export class VkApiClient implements ISocialApiClient {
  ...
  async call<T = any>(method: string, params: Record<string, any>, lease: ApiKeyLease): Promise<{ data: T; statusCode: number; headers?: Record<string, string | number> }> {
    ...
    try {
      const res = await axios.get(`${url}?${query.toString()}`, { validateStatus: () => true });
      if (res.status >= 400 || (res.data && res.data.error)) {
        const err: AxiosError = new AxiosError('VK API error', String(res.status), undefined, undefined, res);
        throw err;
      }
      return { data: res.data?.response ?? res.data, statusCode: res.status, headers: res.headers as any };
    } catch (e: any) {
      if (e.response) {
        // Surface HTTP error with status and headers
        const r = e.response;
