...
export interface LoadWallByOwnerParams {
  ownerId?: number;
  domain?: string;
  offset?: number;
  count?: number;
...
@Injectable()
export class LoadWallByOwnerCommand {
  private readonly logger = new Logger(LoadWallByOwnerCommand.name);
    ...
    try {
      const handleRange = async (
        ...
        for (let offset = startOffset; ; offset += pageSizeLocal) {
          ...
          // publish domain event for this chunk
          if (success > 0 && ownerId) {
            const chunkKeys = unique
              .slice(0, success)
              .map((p) => p._key as string)
              .filter(Boolean);
            await this.eventBus.publish(
              new WallPostsImportedEvent(ownerId, chunkKeys, success),
            );
          }
          ...
          if (typeof total === "number" && offset + requestCount >= total)
            break;
