...
export class VkFriendDto {
  ...
  @ApiPropertyOptional({ description: "Скрыт ли пользователь", example: 0 })
  hidden?: number;

  @ApiPropertyOptional({ description: "Дата рождения", example: "11.03.1990" })
  bdate?: string;
  ...
  @ApiPropertyOptional({ description: "Онлайн ли пользователь", example: 1 })
  online?: number;
  ...
  @ApiPropertyOptional({ description: "Профиль закрыт?", example: false })
  is_closed?: boolean;

  @ApiPropertyOptional({
    description: "Можно ли получить доступ к профилю?",
    example: true,
  })
